...
fn main() -> Result<(), Box<dyn std::error::Error>>
{
    ...
    let mut sheet: Sheet = Sheet::new(0, c as usize, r as usize);
        ...
        match ast {
            ...
            ast::Command::AssignCmd(a, b_ex) => {  //NOTE: All validity checks for addresses will be more complicated when we implement multiple sheets.
                ...
                if col.cells.len() <= a.row
                {
                    ...
                    col.cells.resize_with(a.row + 1, || {p += 1; Rc::new(RefCell::new(Cell::new(ast::Addr{row: p as usize, col: a.col})))});
                }
                drop(col);
                // println!("{:?}", dep_vec);
                for dep in &dep_vec {
                    match dep {
                        ast::ParentType::Single(a_1) => {
                            ...
                            {
                                let mut p = col.cells.len() as u32;
                                col.cells.resize_with(a_1.row + 1, || {p += 1; Rc::new(RefCell::new(Cell::new(ast::Addr{row: p as usize, col: a_1.col})))});
                            }
                            drop(col);
                        },
                        ast::ParentType::Range(a_1, a_2) => {
                            ...
                            for i in a_1.col..=a_2.col {
                                ...
                                {
                                    let mut p = col.cells.len() as u32;
                                    col.cells.resize_with(a_2.row + 1, || {p += 1; Rc::new(RefCell::new(Cell::new(ast::Addr{row: p as usize, col: i})))});
                                }
                                drop(col);
                            }
                        },
            ...
            start = Instant::now();
                ...
                if let Err(_) = evaluate(&mut sheet, &a, &old_func)
                {
